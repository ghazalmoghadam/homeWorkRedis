...
{
    public class ReadAuthorizeFilterAttribute : Attribute, IAuthorizationFilter
    {
        public void OnAuthorization(AuthorizationFilterContext context)
        {
            ...
            using (IRedisClient client = new RedisClient())
            {
                ...
                {
                    string readClaimValue = null;
                    foreach (var claim in cacheData)
                    {
                        if (claim.ContainsKey("CanGetReport"))
                            readClaimValue = claim["CanGetReport"];
                    }

                    if (readClaimValue == "false")
